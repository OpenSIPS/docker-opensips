...
build:
	docker build \
		...
		$(DOCKER_ARGS) \
		--tag="opensips/$(OPENSIPS_IMAGE_NAME):$(OPENSIPS_DOCKER_TAG)" \
		.

start:
	docker run -d --name $(NAME) opensips/$(OPENSIPS_IMAGE_NAME):$(OPENSIPS_DOCKER_TAG)
